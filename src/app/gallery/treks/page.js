                ...
                <div key={index}>
                    <div className="flex justify-between items-center">
                        <div className="flex gap-6 items-center">
                            <div className="text-primary font-bold text-lg md:text-3xl sm:text-lg">{trek.name}</div>
                            <div className="text-gray-600 font-semibold text-sm md:text-lg sm:text-sm">{new Date(trek.start_date).toLocaleDateString()}</div>
                        </div>
                        <div>
                            <Link href={`/gallery/treks/${trek.slug}`}>
                    ...
                    <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 my-8'>
                        {trek.images.slice(0, 4).map((imgObj, innerIndex) => (
